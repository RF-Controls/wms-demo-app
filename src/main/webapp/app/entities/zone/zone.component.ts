...
export class ZoneComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.zoneService
        ...
        .subscribe(
          (res: HttpResponse<IZone[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInZones(): void {
    this.eventSubscriber = this.eventManager.subscribe('zoneListModification', () => this.loadPage());
  }
