...
/**
 * REST controller for managing {@link com.rfcontrols.wms.domain.Item}.
 */
@RestController
@RequestMapping("/api")
public class ItemResource {
    ...
    /**
     * {@code GET  /items/epc/:epc} : get the "epc" item.
     *
     * @param epc the epc of the itemDTO to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the itemDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/items/epc/{epc}")
    public ResponseEntity<ItemDTO> getItemByEpc(@PathVariable String epc) {
        log.debug("REST request to get Item by epc: {}", epc);
        Optional<ItemDTO> itemDTO = itemService.findByEpc(epc);
        return ResponseUtil.wrapOrNotFound(itemDTO);
    }
